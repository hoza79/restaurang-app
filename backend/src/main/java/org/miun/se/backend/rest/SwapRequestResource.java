...
public class SwapRequestResource {
    ...
    public Response create(SwapRequestDto dto) {
        Employee sender = em.find(Employee.class, dto.getSenderId());
        Employee receiver = em.find(Employee.class, dto.getReceiverId());
        Shift shift = em.find(Shift.class, dto.getShiftId());
        ...
        em.persist(swap);
        return Response.status(Response.Status.CREATED).entity(swap).build();
    }

    // GET /api/swap-requests/incoming?receiverId={id}
    @GET
    @Path("/incoming")
    public List<SwapRequest> incoming(@QueryParam("receiverId") Integer receiverId) {
        return em.createQuery(
                        "SELECT s FROM SwapRequest s WHERE s.receiverEmployee.employeeId = :rid", SwapRequest.class)
                .setParameter("rid", receiverId).getResultList();
    }

    // GET /api/swap-requests/outgoing?senderId={id}
    @GET
    @Path("/outgoing")
    public List<SwapRequest> outgoing(@QueryParam("senderId") Integer senderId) {
        return em.createQuery(
                        "SELECT s FROM SwapRequest s WHERE s.senderEmployee.employeeId = :sid", SwapRequest.class)
                .setParameter("sid", senderId).getResultList();
    }
    ...
    public Response accept(@PathParam("swapId") Integer swapId) {
        ...
        return Response.ok(em.merge(swap)).build();
    }
